...
public class VillaListController implements Initializable{
...
//		btnNext.setOnAction(event -> handleBtnNext(event));
		btnSearch.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				try {
					Parent ofcsearch = FXMLLoader.load(AppMain.class.getResource("villalist/villalist.fxml"));
	...
	public ListView<BorderPane> getList(int page) {
		...
		for(int i=1; i<=6; i++) {
			...
			try {
				...
				Image image = new Image(getClass().getResource("../imagesVilla/Villa/"+number+".jpg").toString());
